...
public:
    ...
    bool WriteZREEXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZREEXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZREEXSeed_deprecated(uint256& seed);
    bool EraseZREEXSeed();
    bool EraseZREEXSeed_deprecated();

    bool WriteZREEXCount(const uint32_t& nCount);
    bool ReadZREEXCount(uint32_t& nCount);
