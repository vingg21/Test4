...
// Copyright (c) 2015-2018 The REEXX developers
...
bool CWallet::UpgradeHdChainEncrypted(const SecureString& strWalletPassreexase, const CHDChain& chain)
{
    SecureString strWalletPassreexaseFinal;

    strWalletPassreexaseFinal = strWalletPassreexase;
...

    {
        ...
        for (const MasterKeyMap::value_type& pMasterKey : mapMasterKeys) {
            if (!crypter.SetKeyFromPassreexase(strWalletPassreexaseFinal, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivationMethod))
                return false;
...
bool CWallet::Unlock(const SecureString& strWalletPassreexase, bool anonymizeOnly)
{
    SecureString strWalletPassreexaseFinal;
...
bool CWallet::ChangeWalletPassreexase(const SecureString& strOldWalletPassreexase, const SecureString& strNewWalletPassreexase)
{
    bool fWasLocked = IsLocked();
    SecureString strOldWalletPassreexaseFinal = strOldWalletPassreexase;

    {
        ...
        for (MasterKeyMap::value_type& pMasterKey : mapMasterKeys) {
            if (!crypter.SetKeyFromPassreexase(strOldWalletPassreexaseFinal, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivationMethod))
                return false;
            if (!crypter.Decrypt(pMasterKey.second.vchCryptedKey, vMasterKey))
                return false;
            if (CCryptoKeyStore::Unlock(vMasterKey)) {
                int64_t nStartTime = GetTimeMillis();
                crypter.SetKeyFromPassreexase(strNewWalletPassreexase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivationMethod);
                ...
                LogPrintf("Wallet passreexase changed to an nDeriveIterations of %i\n", pMasterKey.second.nDeriveIterations);

                if (!crypter.SetKeyFromPassreexase(strNewWalletPassreexase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivationMethod))
                    return false;
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassreexase)
{
    if (IsCrypted())
        return false;
    ...
    CCrypter crypter;
    int64_t nStartTime = GetTimeMillis();
    crypter.SetKeyFromPassreexase(strWalletPassreexase, kMasterKey.vchSalt, 25000, kMasterKey.nDerivationMethod);
    kMasterKey.nDeriveIterations = 2500000 / ((double)(GetTimeMillis() - nStartTime));

    nStartTime = GetTimeMillis();
    crypter.SetKeyFromPassreexase(strWalletPassreexase, kMasterKey.vchSalt, kMasterKey.nDeriveIterations, kMasterKey.nDerivationMethod);
    ...
    if (!crypter.SetKeyFromPassreexase(strWalletPassreexase, kMasterKey.vchSalt, kMasterKey.nDeriveIterations, kMasterKey.nDerivationMethod))
        return false;
...
void CWallet::GenerateNewHDChain(const std::string& words, const SecureString& strWalletPassreexase)
{
    CHDChain newHdChain;
    ...
    if(mapArgs.count("-hdseed") && IsHex(strSeed)) {
        ...
        if (words.size() !=0) {
            ...
            if(strMnemonic.empty()){
                ...
                std::string notice = "This is your seed reexase, please write it down to recover wallet \n\"" + join(vectorWords," ")+"\"\n" +
                                     "NEVER SHARE THIS SEQUENCE WITH ANYONE TO PROTECT YOUR FUNDS";
                ShowSeedReexase(notice);
            }
        }

        // NOTE: default mnemonic passreexase is an empty string
        std::string strMnemonicPassreexase = GetArg("-mnemonicpassreexase", "");

        SecureVector vchMnemonic(strMnemonic.begin(), strMnemonic.end());
        SecureVector vchMnemonicPassreexase(strMnemonicPassreexase.begin(), strMnemonicPassreexase.end());

        if (!newHdChain.SetMnemonic(vchMnemonic, vchMnemonicPassreexase, true))
            throw std::runtime_error(std::string(__func__) + ": SetMnemonic failed");
    }
    newHdChain.Debug(__func__);

    if (IsCrypted()) {
        if (!UpgradeHdChainEncrypted(strWalletPassreexase, newHdChain))
            throw std::runtime_error(std::string(__func__) + ": SetCryptedHDChain failed");
    }
    else {
...
void CWallet::GenerateNewHDChain(const std::vector<std::string>& words, const SecureString& strWalletPassreexase)
{
    std::string strMnemonic;
    strMnemonic = join(words," ");
    return GenerateNewHDChain(strMnemonic, strWalletPassreexase);
}
...
{
    return zreexTracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZREEX = GetBoolArg("-zapwallettxes", false);
    if (fCheckZREEX)
        zreexTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zreex
            if (fCheckZREEX && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        vector<CMintMeta> vMints = zreexTracker->GetMints(true);
    ...
    return zreexTracker->GetBalance(false, false);
}
...
{
    return zreexTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zreexTracker->ListMints(true, true, true);
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, int blockHeight) const
{
    ...
    for (const COutput &out : vCoins) {
        ...
        std::unique_ptr<CRetrexStake> input(new CRetrexStake());
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, GetBudgetSystemCollateralAmount(chainActive.Height()))); // Old 50 REEX collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, param->coinSelectStrategy, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 REEX.");
                    } else {
...
bool CWallet::CreateZREEXOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZREEX(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZREEXOutput(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zreex output", __func__);
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
        ...
        if (!zerocoinSelected.GetKeyPair(key))
            return error("%s: failed to set zREEX privkey mint version=%d", __func__, nVersion);
    ...
    try {
        ...
        //Add the coin spend into a REEX transaction
        ...
        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            ...
            if (!zreexTracker->HasSerialHash(hashSerial))
                return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

            CMintMeta meta = zreexTracker->Get(hashSerial);
            meta.isUsed = true;
            if (!zreexTracker->UpdateState(meta))
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CTxDestination* address)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zREEX"), nStatus);
        return false;
    }
    ...
    if (vSelectedMints.empty()) {
        setMints = zreexTracker->ListMints(true, true, true); // need to find mints to spend
        if(setMints.empty()) {
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = ZREEX_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zREEX require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zreexTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zreexTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zreexTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zreexTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

string CWallet::GetUniqueWalletBackupName(bool fzreexAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzreexAuto ? "-autozreexbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}
...
void CWallet::ZReexBackupWallet()
{
    ...
    if(!GetArg("-zreexbackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zreexbackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CTxDestination* addressTo)
{
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zreexTracker->SetPubcoinNotUsed(hashPubcoin);
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zreexTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zreexTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
...
{
    CMintMeta meta;
    if (!zreexTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zreexTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zreexTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zreexTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zreexTracker->UpdateState(meta);
    } else {
