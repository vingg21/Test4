...
bool WalletModel::setWalletEncrypted(bool encrypted, const SecureString& passreexase)
{
    if (encrypted) {
        // Encrypt
        return wallet->EncryptWallet(passreexase);
    } else {
...
bool WalletModel::setWalletLocked(bool locked, const SecureString& passReexase, bool anonymizeOnly)
{
    if (locked) {
        // Lock
        ...
        // Unlock
        return wallet->Unlock(passReexase, anonymizeOnly);
    }
}
...
bool WalletModel::changePassreexase(const SecureString& oldPass, const SecureString& newPass)
{
    bool retval;
    {
        LOCK(wallet->cs_wallet);
        wallet->Lock(); // Make sure wallet is locked before attempting pass change
        retval = wallet->ChangeWalletPassreexase(oldPass, newPass);
    }
    return retval;
}
...
{
    setMints.clear();
    setMints = pwalletMain->zreexTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
