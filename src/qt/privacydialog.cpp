...
void PrivacyDialog::on_pushButtonMintzREEX_clicked()
{
    ...
    if (encStatus == walletModel->Locked) {
        ...
        if (!ctx.isValid()) {
            ...
            ui->TEMintStatus->setPlainText(tr("Error: Your wallet is locked. Please enter the wallet passreexase first."));
            return;
        }
    }
...
void PrivacyDialog::sendzREEX()
{
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zREEX was spent successfully update the addressbook with the label
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zreexTracker->GetMints(false);
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzREEX->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zREEX
        ui->pushButtonMintzREEX->setEnabled(false);
        ui->pushButtonMintzREEX->setToolTip(tr("zREEX is currently disabled due to maintenance."));

        // Spend zREEX
        ui->pushButtonSpendzREEX->setEnabled(false);
        ui->pushButtonSpendzREEX->setToolTip(tr("zREEX is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zREEX
        ui->pushButtonMintzREEX->setEnabled(true);
        ui->pushButtonMintzREEX->setToolTip(tr("PrivacyDialog", "Enter an amount of REEX to convert to zREEX", 0));

        // Spend zREEX
        ui->pushButtonSpendzREEX->setEnabled(true);
        ui->pushButtonSpendzREEX->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}