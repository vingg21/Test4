...
VERSION=
commit=false
url=https://github.com/retrexproject/retrex
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the retrex, gitian-builder, gitian.sigs, and retrex-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/retrexproject/retrex
...
then
	# Make output folder
	mkdir -p "./retrex-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit retrex=${COMMIT} --url retrex=${url} ../retrex/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../retrex/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/retrex-*.tar.gz build/out/src/retrex-*.tar.gz ../retrex-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit retrex=${COMMIT} --url retrex=${url} ../retrex/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/retrex-*.zip build/out/retrex-*.exe ../retrex-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit retrex=${COMMIT} --url retrex=${url} ../retrex/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/retrex-*.tar.gz build/out/retrex-*.dmg ../retrex-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit retrex=${COMMIT} --url retrex=${url} ../retrex/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../retrex/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/retrex-*.tar.gz build/out/src/retrex-*.tar.gz ../retrex-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../retrex/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/retrex-*win64-setup.exe ../retrex-binaries/${VERSION}
	    mv build/out/retrex-*win32-setup.exe ../retrex-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../retrex/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/retrex-osx-signed.dmg ../retrex-binaries/${VERSION}/retrex-${VERSION}-osx.dmg
	fi
	popd || exit
